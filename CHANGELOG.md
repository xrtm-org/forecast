...
### Fixed
- Backfilled missing analyst `confidence_interval` values so the provider-free example remains schema-clean when paired with the released `xrtm` 0.3.0 deterministic provider payload.

## [0.6.6] - 2026-04-30

### Fixed
- Hardened provider runtime semantics, streaming cleanup, deterministic unsupported streaming errors, and rate-limiter timeout/cancellation behavior.
